...
int main()
{
    ...
    float pl_times[NUM_SAMPLES];
    float ps_total_time = 0;
    float pl_total_time = 0;
    ...
    while (status != 0x00000002)
       ...
       status = XAxiDma_ReadReg(XPAR_AXI_DMA_0_BASEADDR, 0x34) & 0x00000002;
        ...
        float pl_time = (float)(pl_end - pl_start) / (COUNTS_PER_SECOND / 1000000);
        pl_times[s] = pl_time;
        pl_total_time += pl_time;

        /*******************************  Comparison logic  ********************** */
        float diff1 = pl_output[0] - ps_res[0];
        float diff2 = pl_output[1] - ps_res[1];
        int error=0;
        if(diff1>0.1 || diff2>0.1){
            error++;
            printf("PS PL  mismatch\n");
            return 1;
        }
        else{
            printf(" sample %d : Ps Pl matches \n",sample->sample_id);
        }

        printf("%-5d | %-5d | (%5.6f, %5.6f)           | (%5.6f, %5.6f)            \n",
               sample->sample_id, sample->snr, ps_res[0], ps_res[1] ,pl_output[0] , pl_output[1] );
    ...
    float pl_average = pl_total_time / NUM_SAMPLES;
    printf("------------------------------------------------------------\n");
    printf("Average PL Execution Time: %f us\n", pl_average);

    printf("\n --- Overall Comparison ---\n");
    printf("PS Average: %f us\n", ps_average);
    printf("PL Average: %f us\n", pl_average);
    printf("Speedup:    %.2f x\n", ps_average / pl_average);
