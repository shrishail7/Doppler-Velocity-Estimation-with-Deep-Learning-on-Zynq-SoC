...
void matmul_complex(const Complex *A, const Complex *B, Complex *C, int n) {
    ...
    // Clear C first
    MATMUL_CLEAR_I: for(int i=0; i<n; i++) {
        int i_n = i * n;
        MATMUL_CLEAR_J: for(int j=0; j<n; j++) {
    ...
    MATMUL_I: for (int i = 0; i < n; i++) {
        int i_n = i * n;
        MATMUL_K: for (int k = 0; k < n; k++) {
            float ar = A[i_n + k].real, ai = A[i_n + k].imag;
            int k_n = k * n;
            MATMUL_J: for (int j = 0; j < n; j++) {
                #pragma HLS PIPELINE II=1
                float br = B[k_n + j].real, bi = B[k_n + j].imag;
                float cr = C[i_n + j].real, ci = C[i_n + j].imag;

                C[i_n + j].real = cr + (ar * br - ai * bi);
                C[i_n + j].imag = ci + (ar * bi + ai * br);
            }
        }
    }
}
...
void copy_matrix(Complex *src, Complex *dst, int n) {
    #pragma HLS INLINE off
    COPY_MAT_I: for(int i=0; i<n*n; i++) {
        #pragma HLS PIPELINE II=1
        dst[i] = src[i];
    }
}
...
void identity_matrix(Complex *A, int n) {
    #pragma HLS INLINE off
    IDENTITY_I: for(int i=0; i<n*n; i++) {
        #pragma HLS PIPELINE II=1
        A[i].real = 0.0f; A[i].imag = 0.0f;
    }
    IDENTITY_J: for(int i=0; i<n; i++) {
        #pragma HLS PIPELINE II=1
        A[i*n+i].real = 1.0f;
    }
}
...
void qr_givens_S(const Complex *A, int n, Complex *Q_out, Complex *R_out) {
    ...
    QR_COL: for (int i = 0; i < n - 1; i++) {
        int i_n = i * n;
        QR_ROW: for (int j = i + 1; j < n; j++) {
            int j_n = j * n;
            Complex cosf, sinf;
            givensrotation_new(R_out[i_n + i], R_out[j_n + i], &cosf, &sinf);
...
void qr_algorithm_eig_single(const Complex *A_input, int n, int max_iterations, Complex *eigenvalues, Complex *eigenvectors) {
    ...
    QR_ITER: for (int iter = 0; iter < max_iterations; iter++) {
        qr_givens_S(A, n, Q, R);
